...
When the owner sends a bulk investor update or broadcast (sourced from a single `todo.md` task like "Email existing investors"), the inboxes produce **N independent reply stubs** from different recipients. Each needs separate triage within the same parent project.
...
| **Resolved by owner / thread terminal** | the owner already replied to the investor (asked "what are you up to", made a closing remark) and the investor's reply is just conversational closure. No remaining ask on either side. | File under journal only — no todo row, no new action. The thread is naturally resolved. Mark the owner's todo row (e.g. "Email existing investors") as noting a response from this recipient, not as needing follow-up. |
