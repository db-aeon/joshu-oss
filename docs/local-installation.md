...
detailed Hermes integration notes in `docs/hermes-customizations.md`.
...
```text
/Users/danbenyamin/Documents/dev/joshu
```
...
# LLM via OpenRouter (verified local default — see docs/hermes-customizations.md)
...
```text
/Users/danbenyamin/Documents/dev/hermes-agent/venv/bin/python3 \
  /Users/danbenyamin/Documents/dev/hermes-agent/venv/bin/hermes \
...
Joshu’s [`scripts/update-hermes-agent.sh`](scripts/update-hermes-agent.sh) installs Hermes with the same **pip extras as Modal** (`HERMES_MODAL_EXTRAS`), including **`voice`** and **`messaging`** — full-turn CLI voice (`/voice on`, Ctrl+B), plus the gateway stack for Discord/Telegram when you enable it.
...
**Modal:** Prefer Groq (or another cloud STT) on CPU workers instead of heavy local Whisper. Put keys in Modal’s secret and load them into `/root/.hermes/.env` via **`HERMES_ENV_B64`** (see [`scripts/modal-start.sh`](scripts/modal-start.sh)) — do not commit secrets into the repo-tracked [`.hermes/`](.hermes) snapshot. Override **`stt.provider`** for the container in that env-backed config if needed.
...
**Broader local venv** (optional): set **`HERMES_LOCAL_EXTRAS`** when running the update script, e.g. `all,dev` for full upstream dev — default remains Modal parity.

### Langfuse observability (optional)

Full setup, env keys, and trace verification live in
[`docs/hermes-customizations.md`](hermes-customizations.md) (**Langfuse observability**).
...
possible. Do not put Joshu-owned behavior in `/Users/danbenyamin/Documents/dev/hermes-agent`;
...
`docs/hermes-customizations.md` (section **Updating Hermes**).

### Recommended: Joshu update script

From the Joshu repo, use the scripted path to promote the latest upstream **stable
release**, refresh the Hermes venv, update the Modal pin, and keep a local
rollback snapshot:
...
npm run modal:deploy        # full in-image Node build (slow; good for CI parity)
npm run modal:deploy:fast   # local dist + MODAL_EMBED_LOCAL_DIST (when only Joshu changed; see docs/hitl-camofox-modal-notes.md)
```
...
```bash
cd /Users/danbenyamin/Documents/dev/hermes-agent
git status
...
The Joshu script snapshots the **checkout**, **venv packages**, and **Modal pin**.
...
```bash
/Users/danbenyamin/Documents/dev/hermes-agent/venv/bin/python - <<'PY'
...
```bash
cd /Users/danbenyamin/Documents/dev
...
```text
/Users/danbenyamin/Documents/dev/hermes-agent-next/venv/bin/hermes
```
...
```text
/Users/danbenyamin/Documents/dev/hermes-agent/venv/bin/hindsight-api
```
...
exercise the same subservice paths used by the Modal package. **Schedules** manages Hermes cron jobs (`/joshu/api/cron/*`); see [`docs/schedules-arozos-app.md`](schedules-arozos-app.md). **Hermes Admin** (Kanban, cron, skills, MCP) opens at `/joshu/hermes-admin/` — desktop shortcut or `http://127.0.0.1:8788/joshu/hermes-admin/`; requires `HERMES_BIN` in `.env` and dashboard on `:9119` ([hermes-customizations — local dashboard](hermes-customizations.md#local-dev-joshu-subpath-proxy)). Stock shortcuts use
...
[`hitl-camofox-modal-notes.md`](hitl-camofox-modal-notes.md#vnc-display-routing-and-troubleshooting-resolved-may-2026).

### UI / design system

- In-app surfaces use [`packages/design-system`](../packages/design-system) via
  `@joshu/design-system` (tokens, typography, base CSS).
- ArozOS desktop chrome is overlaid by
  [`arozos/web-overlays/aroz-paper-shell.css`](../arozos/web-overlays/aroz-paper-shell.css);
...
- Shell assets: `arozos/web-overlays/` (CSS, taskbar focus JS, desktop icon tooltips, overlay guard); Tango PNGs → `web/img/joshu/` (`arozos/icons/`), `web/img/desktop/` (`arozos/desktop-icons/`), and `web/img/tango/` (full 256×256 library in `arozos/tango-icons/`). Rebuild: [`docs/design/README.md`](design/README.md#tango-icon-pipeline).
...
The Modal image differs in a few important ways: local PostgreSQL data lives under
ephemeral `/tmp` in the container, the web `serve` function uses a long timeout and
scaledown window (see `modal_app.py`), and pgvector is built with portable compiler
flags. See `README.md` (Optional Modal Target) and `docs/hitl-camofox-modal-notes.md`
...
```bash
ps -axo pid,ppid,user,command | rg -i '(/Users/danbenyamin/Documents/dev/hermes-agent|hermes gateway|hindsight-api)'
```
