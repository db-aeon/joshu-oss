...
| [`arozos/web-overlays/aroz-desktop-overlay-guard.js`](../../arozos/web-overlays/aroz-desktop-overlay-guard.js) | Clears stuck notification/drag layers; multi-tab stale-tab recovery; console helpers |
...
- [`modal_app.py`](../../modal_app.py) — during image build on `/opt/arozos-template/web`

Joshu subservices set `IconPath` in `moduleInfo.json` (e.g. `img/joshu/chat.png`). `scripts/dev-arozos.sh` and `scripts/modal-start.sh` also rewrite **stock** desktop shortcuts (File Manager, System Setting, Trash Bin) and Joshu app shortcuts to `img/joshu/*` on every data prepare. Stock ArozOS **file-type** icons under `vendor/arozos/src/web/img/desktop/` are **overridden at runtime** by [`arozos/desktop-icons/`](../../arozos/desktop-icons/) (see [Tango icon pipeline](#tango-icon-pipeline)).
...
**File Manager:** `apply_arozos_joshu_theme.py` patches `SystemAO/file_system/file_explorer.html` to use `img/joshu/folder.png` in list/details/grid instead of Semantic UI folder glyphs.
...
| Desktop `.shortcut` files | Lines 1–3 = type / display name / module path; line 4 = icon | Installed by `dev-arozos.sh` / `modal-start.sh` — see [`docs/arozos-desktop-shortcuts.md`](../arozos-desktop-shortcuts.md) |
...
**Reference subservice:** [`arozos/subservice/icon-test/`](../../arozos/subservice/icon-test/) — minimal static app using `img/joshu/icon-test.png`; registered in `dev-arozos.sh` (not required for production Modal unless you add it to `modal_app.py`).
...
- Modal / ArozOS topology: [`docs/hitl-camofox-modal-notes.md`](../hitl-camofox-modal-notes.md)
