...
```text
control plane: https://joshu-beige.vercel.app
...
- Add `apps/control-plane` to `pnpm-workspace.yaml`.
...
that Modal had been injecting implicitly:
...
The repository still contains Modal-era deploy files and docs:

- `modal_app.py`
- `scripts/modal-start.sh`
- `scripts/modal-create-secret.sh`
- `docs/hitl-camofox-modal-notes.md`
- `docs/vps-sandbox/modal-to-vps-mapping.md`
- npm scripts such as `modal:predeploy`, `modal:deploy`, and
  `modal:deploy:fast`
...
1. Rename the shared bundle build step from `modal:predeploy` to a neutral name
   such as `build:bundles` or `sandbox:prebuild`.
2. Make `vps:predeploy` call that neutral script.
3. Update VPS docs to avoid Modal naming for production builds.
4. Mark Modal docs as legacy/demo-only.
5. Remove `modal:deploy`, `modal:deploy:fast`, `modal:secret`,
   `scripts/modal-create-secret.sh`, and eventually `modal_app.py` after at
