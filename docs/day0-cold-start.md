...
| [`src/day0/llm.ts`](../src/day0/llm.ts) | OpenRouter chat; Langfuse generations when `HERMES_LANGFUSE_*` set (trace `joshu-day0-infer` / `joshu-day0-sweep`) |
...
- EA playbook skill — new-engagement pointer to Day 0 button

## Follow-ups (out of v1)

- Wire `day0/sweep` into Hermes triage-window cron prompts
- Auto-open Welcome to Big Picture step after Day 0
- Sync inferred timezone → `HERMES_TIMEZONE` in Schedules
