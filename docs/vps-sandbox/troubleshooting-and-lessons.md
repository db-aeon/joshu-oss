...
14. **Joshu API crash: `Cannot find module .../dist/connectors/routes.js`** — `npm run build:connectors` (Vite) used to write to `dist/connectors` with `emptyOutDir: true`, wiping `tsc` output (`routes.js`, etc.). UI now builds to `dist/connectors-app/`.  
    **Fix:** Rebuild image after that change (`modal:predeploy` → `vps:build-image`); on a running box, `docker exec deploy-joshu-stack-1 test -f /opt/joshu/dist/connectors/routes.js`.
...
Modal parity (`scripts/modal-start.sh`) exports `ENABLE_VNC` and `NOVNC_*`; VPS bootstrap and `sandboxEnv.ts` now match.
...
See [hitl-camofox-modal-notes.md](../hitl-camofox-modal-notes.md) for VNC env table, debug overlay, and noVNC cursor pitfalls.
...
- [hitl-camofox-modal-notes.md](../hitl-camofox-modal-notes.md) — VNC viewer, fit-viewport, Camofox patch
